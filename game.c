...
void para_init(Parameter *para) {
  ...
  para->tick = 0;
  para->platform_generation_tick = 60;
...
void update_game(float time) {
  ...
  if (game.state == Gaming &&
      game.para.tick % game.para.platform_generation_tick == 0) {
    int platform_type;
...
void action_push(Action a) { action_queue_push(&game.aq, a); }

NodePlayer *query_has(int fd) { return _query_has(&game.ll_player, fd); }
...
static void calculate_damage() {
  ...
  while (player) {
    ...
    NodePlatform *platform = game.ll_platform.head;
    while (platform) {
      if (platform->p.type == Spike &&
          rect_is_on(&player->p.rect, &platform->p.rect)) {
